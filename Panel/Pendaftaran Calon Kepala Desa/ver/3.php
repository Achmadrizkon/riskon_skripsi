...
<?php 
$no = 1;
$sql = mysqli_query($koneksi,"SELECT * FROM seleksi 
...
$id = $data['id_seleksi'];
$ss = mysqli_query($koneksi,"SELECT * FROM calon WHERE id_seleksi='$id'");
...
<?php if ($level_user == "Administrator"): ?>
    <?php 
        ...
        ?>
<td align="center">
<a href="Calon Kepala Desa/proses.php?id=<?=$id;?>" class="btn btn-icon btn-success">
...
<td><?=tgl($data['tanggal_seleksi']);?></td>
<?php 
    if ($jml >= 1) {
    ?>
        <td align="center">
    ...
    ?>
<td align="center">
<a href="index.php?page=Seleksi Calon Kepala Desa&form=Hapus&id=<?=$id;?>" class="btn btn-icon btn-danger" onclick="return confirm('Yakin ingin menghapus data?')">
