 ...
 <?php 
 if ($_GET['form'] == "Ubah") {
    $sql    = mysqli_query($koneksi,"SELECT * FROM pembukaan_pendaftaran_aparat WHERE id_pembukaan_pendaftaran_aparat='$id'");
	...
	<label class="form-label">Pangkat Aparat</label>
	...
	<select name="status_pembukaan_pendaftaran_aparat"  class="form-control select2-show-search form-select" data-placeholder="Pilih Salah Satu" id="status_pembukaan_pendaftaran_aparat" required>
	<option value>-- Pilih Status Pembukaan --</option>
	<?php
	if ($data['status_pembukaan_pendaftaran_aparat'] == NULL){

		}else{
			?>
			<option value="<?=$data['status_pembukaan_pendaftaran_aparat'];?>" selected><?=$data['status_pembukaan_pendaftaran_aparat'];?></option>		
			<?php
		}
		?>
				<?php
				if($data['status_pembukaan_pendaftaran_aparat'] == "Mulai"){
					...
					;?>
				<?php
				if($data['status_pembukaan_pendaftaran_aparat'] == "Selesai"){
